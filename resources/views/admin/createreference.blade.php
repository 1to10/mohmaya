...
@section('page_heading',(isset($categoryByid))?'Update Reference':'Add Reference')
@section('section')
    @if($errors->any())
        ...
        <div class="row">
            <div class="col-sm-12">
                @component('admin.widgets.panel')
                    @slot('panelTitle', (isset($categoryByid))?'Update Reference':'Add Reference')
                    @slot('panelBody')
                          @if(isset($categoryByid))
                            {!! Form::model($categoryByid, [
                          'method' => 'POST',
                          'route' => ['reference.update', $categoryByid->id]
                      ]) !!}
                        @else
                        {!! Form::open(['url' => 'admin/storereference']) !!}
                        @endif
						...
						<div class="form-group">
                            {!! Form::label('Reference', 'Name:', ['class' => 'control-label']) !!}
                            {!! Form::text('spouse_name', (isset($categoryByid->spouse_name))?$categoryByid->spouse_name:'', ['class' => 'form-control','Required'=>'required']) !!}
                        </div>
						...
						<div class="form-group">
                            ...
                            {!! Form::text('mobile', (isset($categoryByid->mobile))?$categoryByid->mobile:'', ['class' => 'form-control','Required'=>'required']) !!}
                        </div>
						<div class="form-group">
                            {!! Form::label('profes_detail', 'Professional Detail:', ['class' => 'control-label']) !!}
                            {!! Form::text('profes_detail', (isset($categoryByid->profes_detail))?$categoryByid->profes_detail:'', ['class' => 'form-control','Required'=>'required']) !!}
                        </div>
